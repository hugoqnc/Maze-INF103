...
public class Maze implements GraphInterface
	...
	private static ArrayList<String> caractere(){//liste des caract√®res acceptes
		...
		for (int i=0; i<longeur; i++) {
			...
			for(int j=0; j<largeur; j++) {
				...
				if (designation.contentEquals("E")){
					...
					compteurA++;
					if (compteurA > 1){
						throw new MazeReadingException(fileName, i+1, "Il y a plusieurs cases A, qui doit Ítre unique");
					}
					...
					compteurD++;
					if (compteurD > 1){
						throw new MazeReadingException(fileName, i+1, "Il y a plusieurs cases D, qui doit Ítre unique");
					}
				}
				else {
						throw new MazeReadingException(fileName,i+1,"Il y a un caractËre autre que E,W,A,D dans le fichier texte"); //ligne de 1 ‡ n, et non de 0 ‡ n-1	
				}
				larg.add(box);
			}
			maze.add(larg);
...
*/
	...
	public int getWeight(VertexInterface src,VertexInterface dst) {// √† modifier si graph pond√©r√©.
		Box boxSrc = (Box)src ;
		Box boxDst = (Box)dst ;
		return 1;
	}
		...
		try (BufferedReader bufferedreader = new BufferedReader(new FileReader(fileName))) {
			...
			while ((strCurrentLine = bufferedreader.readLine()) != null) {//trim supprime espaces superflux eventuels en fin de texte
				...
				if (strCurrentLine.length() != largeurLine1) {
					throw new MazeReadingException(fileName,lineNumber,"Cette ligne n'a pas la mÍme longeur que les autres");
				}
